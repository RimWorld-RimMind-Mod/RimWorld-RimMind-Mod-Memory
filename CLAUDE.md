...
**注意**: `darkCount` 参数在 `ApplyPawnDarkMemory`/`ApplyNarratorDarkMemory` 中未实际使用来限制条目数，完全依赖 prompt 约束 AI 输出条数。
...
public class RimMindMemorySettings : ModSettings
{
    ...
    int darkCount = 3;                  // Pawn 暗记忆条数（prompt 约束，代码未强制）
    ...
    float narratorActiveInjectRatio = 0.5f;   // UI 未暴露
    float narratorArchiveInjectRatio = 0.5f;  // UI 未暴露
    ...
    // 聚合
    int minAggregationCount = 2;        // UI 未暴露，WorkSessionAggregator 使用硬编码常量 MinCountForAggregation=2
...
- `minAggregationCount` 在 Settings 中定义但 WorkSessionAggregator 使用硬编码常量 `MinCountForAggregation = 2`，设置项未生效
- `narratorActiveInjectRatio`/`narratorArchiveInjectRatio` 在代码中使用但设置 UI 未暴露控件
- `darkCount` 参数传入 `ApplyPawnDarkMemory`/`ApplyNarratorDarkMemory` 但方法内未使用，暗记忆条数完全依赖 prompt 约束
