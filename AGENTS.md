...
```csharp
// 命名空间: RimMind.Memory.WorkingMemory
...
public class RimMindMemoryWorldComponent : WorldComponent
{
    ...
    Dictionary<int, WorkingMemory> _workingMemories;
    ...
    public WorkingMemory GetOrCreateWorkingMemory(Pawn pawn);
    public WorkingMemory? GetWorkingMemory(Pawn pawn);
    public void ClearWorkingMemory(Pawn pawn);
}
```
...
| 重要工作 | WorkSessionAggregator (SignificantJobs) | 0.5-0.9 (Rescue=0.8, Attack=0.9) | Pawn + 条件升级叙事者 |
| 受伤/患病 | Patch_AddHediff | lethal=0.9, chronic=0.8, tendable=0.7, sick=0.6, else=0.5 | Pawn + 条件升级叙事者 |
| 精神崩溃 | Patch_MentalBreak | Berserk=0.95, Manhunter=0.9, Extreme=0.9, Serious=0.8, else=0.7 | Pawn + 条件升级叙事者 |
| 亲近者死亡 | Patch_PawnKill | 有关系=1.0, 无关系=0.85, 叙事者=1.0 | 相关 Pawn + 叙事者 |
| 技能升级 | Patch_SkillLevelUp | >=15级=0.7, 否则0.5 | Pawn + 条件升级叙事者 |
...
**工作分类**:
- `BlacklistedJobs`（17 个）— 忽略，不记录
- `WhitelistedJobs`（41 个）— 可聚合，连续同类合并
- `SignificantJobs`（12 个）— 直接单独记录，不聚合

**工作标签**: 通过翻译键 `RimMind.Memory.Work.{defName}` 动态查找，`CanTranslate()` 时使用翻译值，否则回退到 defName。重要工作标签使用相同的翻译键模式。
...
2. 通过 `RimMindAPI.RequestStructured` 发送结构化 AI 请求
   - 使用 `ContextRequest` 构建请求参数（NpcId, Scenario, Budget, MaxTokens, Temperature）
   - 使用 `SchemaRegistry.DarkMemoryOutput` 指定输出 Schema
   - 请求回调中解析 JSON 响应
...
public class RimMindMemorySettings : ModSettings
{
    ...
    // 工作记忆
    int workingMemoryCapacity = 10;
}
```

**所有 25 个设置项均已在 UI 中暴露控件**，含 ExposeData 序列化 + UI 控件 + 重置回调，闭环完整。
...
| `RimMind.Memory.WorkingMemory` | WorkingMemory/ | 工作记忆 |
...
3. 调用 `wc.AddPawnMemory()` 写入 Pawn 记忆
...
```csharp
RimMindMemoryAPI.AddMemory("与艾丽斯进行了深度交谈", "Event", Find.TickManager.TicksGame, 0.6f, pawnId: pawn.ThingID);
```
...
**重要工作（不聚合）**: 在 `WorkSessionAggregator.SignificantJobs` 添加 JobDef 名称，在 `SignificantJobImportanceMap` 添加重要度映射，标签翻译键为 `RimMind.Memory.Work.{defName}`（与可聚合工作使用相同的翻译键模式）。
...
- 已有测试: `PawnMemoryStoreTests`（8 个）、`ImportanceDecayTests`（4 个）、`TimeFormatterTests`（7 个）

## 已知问题

详见 `docs/06-problem/RimMind-Memory.md`。
