...
**工作标签**: 通过翻译键 `RimMind.Memory.Work.{defName}` 动态查找，`CanTranslate()` 时使用翻译值，否则回退到 defName。重要工作标签使用相同的翻译键模式 `RimMind.Memory.Work.{defName}`。
...
2. 通过 `RimMindAPI.RequestAsync` 发送 AI 请求
   - System Prompt: 使用 `StructuredPromptBuilder.FromKeyPrefix` 构建（Pawn 用 `"RimMind.Memory.Prompt.System"`，叙事者用 `"RimMind.Memory.Prompt.NarratorSystem"`），均为五段式结构（Role/Goal/Process/Constraint/Example+Output+Fallback）
   - User Prompt: 经 `PromptSanitizer.Sanitize()` 清洗后传入
   - 请求优先级: `AIRequestPriority.Low`
...
```
RimMind-Memory
    ├── RimMind-Core (RimMindAPI, AIRequest, StructuredPromptBuilder, PromptSanitizer, PromptSection, SettingsUIHelper)
